...
function Login() {
  useEffect(() => {
    ...
    if (token) {
      ...
      fetch('https://localhost:8080/api/auth/verify', {
        method: 'POST',
        headers: {
  ...
  const handleGithubLogin = () => {
    window.location.href = 'https://localhost:8080/auth/github';
  };
  ...
  return (
    <div className="login-page">
      <div className="login-container">
        <h1>로그인</h1>
        <p>할일 목록을 사용하려면 로그인해주세요.</p>
        
        <div className="login-buttons">
          <button className="login-btn google-btn" onClick={handleGoogleLogin}>
            <svg className="btn-icon" viewBox="0 0 24 24" width="24" height="24">
